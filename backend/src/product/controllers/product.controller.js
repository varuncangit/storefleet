...
export const getAllProducts = async (req, res, next) => {
  ...
  const { keyword, category, price, minRating, maxRating, page } = req.query;
  ...
  try {
    ...
    if (keyword) {
      ...
      products = await searchProductsRepo(keyword);
      totalCount = products.length;
    } else {
      // Filter products
      const filters = {
        categories: category ? category.split(",") : undefined,
        minPrice: price && price.gte ? parseInt(price.gte, 10) : undefined,
        maxPrice: price && price.lte ? parseInt(price.lte, 10) : undefined,
        minRating: minRating ? parseFloat(minRating) : undefined,
        maxRating: maxRating ? parseFloat(maxRating) : undefined,
      };

      products = await filterProductsRepo(filters);
      totalCount = await getTotalCountsOfProduct();
    }
...
export const getAllReviewsOfAProduct = async (req, res, next) => {
  try {
    ...
    if(product.reviews.length===0){
      return next(new ErrorHandler(200,"No Review found for the product"));
    }
...
export const deleteReview = async (req, res, next) => {
  ...
  try {
    ...
    if (reviewToBeDeleted.user.toString() != req.user._id.toString()) {
      console.log(reviewToBeDeleted.user.toString, req.user._id.toString());
      return next(new ErrorHandler(400, "review by other user"));
    }

    const updatedRating =
      (product.rating * reviews.length - reviewToBeDeleted.rating) /
